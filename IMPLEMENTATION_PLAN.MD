...
> Current implementation status (2026-09-11): the route/page implementation, offline-safe local-font production build, canonical date/deadline normalization, privacy/robots metadata, strict asset/PDF/content validation, fresh-build route/accessibility/SEO audits, CI workflow, atomic Supabase submission/outbox integration, protected Resend dispatcher, multi-viewport Playwright/axe coverage, and Lighthouse release configuration are implemented. `npm run qa:full` is the routine engineering gate. `npm run validate:launch` remains blocked by unverified seed media/PDFs, the empty approval manifest, provider/domain setup, institutional privacy terms, and content-owner approval. The four-day March 18–21 schedule is present and no longer a blocker.
...
|---|---|---|
| **Framework** | Next.js 16.3.4 (App Router) | File-based routing matches the sitemap; static generation (SSG) delivers sub-3s load times on 4G; React ecosystem for components |
| **Styling** | Tailwind CSS 4 | Utility-first approach accelerates repeated card/badge/button components; design tokens map directly to the PRD color/typography system |
| **Content Storage** | Structured JSON files in `/content/` directory | Public copy remains version-controlled Git content; `content/site.json` is canonical for event dates, deadlines, and registration flags |
| **Form Backend** | Supabase/Postgres + private email outbox, Resend worker, and Upstash Redis | Durable atomic persistence and references; queued applicant/Secretariat mail; distributed abuse controls; no payment collection |
...
| **Analytics** | Optional Google Analytics 4 measurement ID | Enabled only after the organizers supply and approve the production measurement ID |
...
- [ ] **Set environment variables** on Vercel for:
  - `SITE_URL` — Canonical public URL for metadata, sitemap, and robots
  - `SUPABASE_URL` / `SUPABASE_SECRET_KEY` — Server-only durable submission and outbox storage
  - `RESEND_API_KEY` / `EMAIL_FROM` / `NOTIFICATION_EMAIL` — Receipt and Secretariat notifications
  - `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` — Distributed rate limiting
  - `RATE_LIMIT_HMAC_SECRET` / `CRON_SECRET` — Server-only abuse and outbox worker secrets
  - `ANALYTICS_MEASUREMENT_ID` — Optional analytics measurement ID, enabled only after approval
...
- [x] All content JSON files exist with valid seed data (Verified: 13/13 validated)
...
- [x] Next.js production build succeeds with 31 application routes generated (`npm run build` passed)
...
### 1.1 Local Font Integration — Per stitch-design-taste & gpt-taste Skills

> Launch note: the network-dependent Google Fonts implementation was removed. Licensed variable WOFF2 files are vendored under `src/assets/fonts/` and loaded through `next/font/local`, so production builds work offline.

**Tasks:**

- [x] **Import Outfit** (weights: 400–800) as the primary heading/display font from the bundled local WOFF2 file
- [x] **Import Inter** (weights: 400–700) as body text from the bundled local WOFF2 file — acceptable for body per skill rules, banned only for display/heading use
- [x] **Import JetBrains Mono** (weights: 400–600) for metadata, timestamps, and monospaced content from the bundled local WOFF2 file
- [x] Configure in `src/app/layout.tsx` as CSS variables that Tailwind references, using Next.js `next/font/local`
...
- [x] Unique sequential Reference ID generation (`REG-GIMUN-2027-XXXX` / `REG-MOOT-2027-XXXX`)
...
- [x] Managed durable persistence through Supabase/Postgres with atomic reference counters; local memory is test-only
...
- [x] Five categorized accordion sections: General, Registration & Fees, GIMUN-Specific, Moot Cup-Specific, Logistics
- [x] On-page search/filter using `SearchInput`
- [x] Closing prompt linking to Contact page
- [x] Accordion uses Framer Motion spring height animation

### 4.2 Venue & Travel Page (`/about/venue`) — PRD §18.2

- [x] Embedded map, transport guidance, accommodation info, local essentials

### 4.3 Organizing Team Page (`/about/team`) — PRD §15.3

- [x] Grouped sections: GIMUN Secretariat, Moot Cup Convening Committee, Organizing Committee
- [x] Profile cards with Double-Bezel treatment, brand-palette initial avatars (not generic gray icons)
- [x] Staggered entrance animation
- [x] Data source: `content/team.json`

### 4.4 Sponsors & Partners Page (`/about/sponsors`) — PRD §18.3

- [x] Why Sponsor Us section (data-driven, no AI cliches)
- [x] Sponsorship pitch deck download (prominent, never buried)
- [x] Tiered sponsor display: logos grayscale by default, color on hover (per brandkit)
- [x] Graceful empty state: pure pitch page if no sponsors confirmed

### 4.5 Gallery & Press Page (`/about/gallery`) — PRD §18.4

- [x] **Masonry/bento grid** using CSS Grid (per stitch-design-taste, NOT uniform grid)
- [x] Lightbox with spring-physics entrance
- [x] Press mentions as link cards
- [x] Images served via Next.js `<Image>` with lazy loading and WebP (per imagegen-frontend-mobile)

### 4.6 About Page (`/about`) — PRD §12.2

- [x] Event story, host institution note, navigation cards to sub-pages

### 4.7 Phase 4 Acceptance Criteria

- [x] FAQ search works instantly, sponsors page professional even with zero sponsors
- [x] Gallery loads fast (lazy loading), all pages responsive
- [x] **Team cards use staggered entrance**, **gallery uses masonry/bento layout**
- [x] **All icons are Lucide SVGs** — no emoji
...
- [x] Reverse-chronological feed with timestamps in `font-mono text-meta`
- [x] Pinning support, track filter with sliding highlight
- [x] No commenting — one-way broadcast
- [x] Staggered `fadeUp` entrance for announcements
- [x] Homepage banner integration from single data source

### 5.2 Results & Awards Page (`/results`) — PRD §19.2

- [x] Pre-event: placeholder message. Post-event: GIMUN + Moot Cup awards sections
- [x] Optional winner photos in masonry gallery

### 5.3 Content Update Workflow Validation (PRD §20, §22.4)

- [x] "11pm Schedule Change" test — full end-to-end workflow verified
- [x] `CONTENT_UPDATE_GUIDE.md` created with step-by-step instructions

### 5.4 Phase 5 Acceptance Criteria

- [x] Announcements appear on both the page and homepage banner from single data entry
- [x] Results page handles both pre/post-event states
- [x] Content update workflow completes in under 10 minutes
