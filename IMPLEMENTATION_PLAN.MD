...
- [ ] **Install Node.js** (LTS v20+) and verify with `node -v` and `npm -v`
- [ ] **Install Git** and configure with the maintainer's GitHub credentials
- [ ] **Create a new GitHub repository** named `gimun-mootcup-website` (private initially, can be made public later)
  - Add a `.gitignore` for Node.js/Next.js projects
  - Add a `README.md` with project overview and quick-start instructions
...
- [ ] **Scaffold the Next.js project** using the App Router:
  ```bash
  npx create-next-app@latest gimun-mootcup-website --typescript --tailwind --eslint --app --src-dir
  ```
- [ ] **Install additional dependencies**:
  ```bash
  npm install lucide-react    # Icon library (lightweight, tree-shakeable)
  npm install clsx            # Conditional class names utility
  npm install date-fns        # Lightweight date manipulation for countdown/schedule
  npm install gray-matter     # Parse Markdown front-matter (if using .md content files)
  npm install sharp           # Image optimization (Next.js uses this internally)
  npm install framer-motion   # Spring-physics animation library — per framer-motion-animator skill
  ```
...
- [ ] **Create `.env.example`** file listing all required env vars (committed to repo as documentation)
...
- [ ] Running `npm run dev` launches the site locally at `localhost:3000`
- [ ] Pushing to `main` branch triggers automatic deployment to the live custom domain
- [ ] All content JSON files exist with valid placeholder data
- [ ] TypeScript compiles with zero errors
- [ ] The live site shows HTTPS (no security warnings)
- [ ] At least one backup collaborator has GitHub repository access
- [ ] Framer Motion is installed and importable
- [ ] Motion config file (`src/lib/motion.ts`) exists with all shared animation variants
...
- [ ] `src/lib/metadata.ts` — generates Next.js `Metadata` objects with OG/Twitter Card tags
- [ ] Default OG image: branded 1200x630 in `/public/images/og/default.jpg`
- [ ] `src/app/sitemap.ts` — generates `sitemap.xml`

### 1.10 Phase 1 Acceptance Criteria

- [ ] Navbar renders correctly on desktop and mobile with all nav items + Register button
- [ ] **Navbar floats** with rounded corners and glass effect (per design-taste-frontend)
- [ ] Register button visible at all times on mobile
- [ ] Dropdown menus animate with spring-physics scale transition
- [ ] **Mobile menu uses staggered reveal** — 80ms cascaded delays
- [ ] Hamburger morphs to X smoothly
- [ ] Announcement banner works correctly with dismiss persistence
- [ ] Footer displays on every page with all required links
- [ ] Countdown chip computes correctly with number animation
...
1. **Hero Section — Per stitch-design-taste "Asymmetric Structure"**
   - [ ] Left-aligned text block (60% width desktop) with generous right whitespace. NOT centered
   - [ ] Event name, dates, venue from `site.json` — `text-display` fluid typography, `font-heading`, `tracking-tight`
   - [ ] Two CTAs: "Register for GIMUN" (orange) and "Register for Moot Cup" (teal) — full-width on mobile
   - [ ] Countdown chip adjacent to hero title
   - [ ] Background: deep indigo gradient with subtle noise texture. NO purple neon, NO floating blobs
   - [ ] Staggered entrance: headline (0ms), subtitle (100ms), CTAs (200ms)
   - [ ] Uses `min-h-[100dvh]`

2. **Dual Event Introduction — Per brandkit Visual Parity**
   - [ ] **Asymmetric bento grid**: `grid-template-columns: 7fr 5fr` — NOT identical boxes
   - [ ] Each block: 2-3 sentences (no AI cliches), track badge, accent left border, "Learn more" link
   - [ ] Visual parity check: both blocks feel equally prominent despite asymmetric sizing
   - [ ] Scroll reveal entrance

3. **Key Dates Strip**
   - [ ] Horizontal timeline showing registration deadline, document release, conference dates, results
   - [ ] Reads from `site.json` (same source as Schedule page)
   - [ ] Staggered entrance: left to right with 80ms delays

4. **Latest Announcement**
   - [ ] Compact card from `announcements.json` — gracefully absent if empty
   - [ ] Uses `ContentCard` component

5. **Social Proof Section** (optional)
   - [ ] Stats with number ticker animation on viewport entry
   - [ ] Omit if no past-edition data exists

6. **Sponsor Logo Strip**
   - [ ] `SponsorStrip` component with marquee animation
   - [ ] Links to `/about/sponsors`

### 2.2 GIMUN Overview Page (`/gimun`) — PRD §15.2

1. **Track-Specific Hero** — orange accent, asymmetric left-aligned, "Register for GIMUN" CTA
2. **Format Explainer** — jargon-free MUN explanation, no AI cliches
3. **Eligibility Block** — who can apply, ContentCard with Double-Bezel treatment
4. **Fee Block** — no-payment language (PRD §6.2), no payment gateway references
5. **Key Dates** — GIMUN-specific from `site.json`
6. **Navigation Links** — to Committees and Rules pages
7. **Primary Register CTA** — full-width orange button

### 2.3 Moot Cup Overview Page (`/moot-cup`) — PRD §15.2

Mirrors GIMUN structure with **teal accent** and Moot Cup content. Same layout patterns, different track color.

### 2.4 Committees Page — GIMUN (`/gimun/committees`) — PRD §16.1

- [ ] **Asymmetric bento grid**: `2fr 1fr 1fr` / `1fr 1fr 2fr` alternating rows (NOT 3 equal columns)
- [ ] Each card: committee name, type badge, topic teaser, chair names, background-guide download
- [ ] Filter by committee type
- [ ] Staggered card entrance with 80ms delays
- [ ] **Detail View**: full topics, chair bios, PDF download with version date in `font-mono`, country matrix, capacity note, register CTA

### 2.5 Problem Categories Page — Moot Cup (`/moot-cup/categories`) — PRD §16.2

- [ ] Category entries with area of law, description, proposition download
- [ ] Version/date stamps in `font-mono text-meta`

### 2.6 Rules & Memorials Page — Moot Cup (`/moot-cup/rules`) — PRD §16.2

- [ ] Memorial formatting rules, submission deadline, round structure, judging criteria

### 2.7 Clarifications Log — Moot Cup (`/moot-cup/clarifications`) — PRD §16.2

- [ ] Numbered chronological list with search/filter
- [ ] Sequential numbers in `font-mono`
- [ ] Submission form for new questions
- [ ] ScrollReveal entrance for each entry

### 2.8 Rules of Procedure — GIMUN (`/gimun/rules`) — PRD §16.1

- [ ] PDF download + on-page readable version

### 2.9 Schedule Page (`/schedule`) — PRD §16.3

- [ ] Day-by-day tabs with Framer Motion `layoutId` sliding indicator
- [ ] Filter bar with sliding highlight
- [ ] Time ranges in `font-mono` for tabular alignment
- [ ] "Updated" tag with `animate-pulse-dot`
- [ ] "Happening Now" highlight with pulsing red dot (real-time, no manual updates)
- [ ] Mobile-first layout with 44px+ touch targets

### 2.10 Resource Hub (`/resources`) — PRD §17.1

- [ ] Filter by track and document type, text search
- [ ] File metadata in `font-mono text-meta`
- [ ] Download opens in new tab
- [ ] Asymmetric grid layout (per stitch-design-taste)

### 2.11 Phase 2 Acceptance Criteria

- [ ] All 10+ pages render with placeholder content, responsive
- [ ] All navigation links work
- [ ] Consistent track color coding
- [ ] No payment references
- [ ] Content changes require only JSON edits
- [ ] **All sections use ScrollReveal** for entrance animations
- [ ] **No banned layout patterns** (no 3-equal-card, no centered heroes, no floating blobs)
- [ ] **No AI cliche copy** anywhere
- [ ] **Homepage hero uses asymmetric layout**
...
- [ ] Two Double-Bezel cards: GIMUN (orange) and Moot Cup (teal)
- [ ] Hover interaction: lift + shadow elevation
- [ ] Smart routing via URL query parameter: `/register?track=gimun`
- [ ] Staggered card entrance (100ms delay between)

### 3.2 GIMUN Registration Form — PRD §17.2

**Form Styling — Per stitch-design-taste:**
- [ ] Labels above inputs, `0.5rem` gap between label-input-error stack
- [ ] Inputs: `rounded-card`, `border-whisper-border`, `px-4 py-3`
- [ ] Focus: accent orange ring, `2px offset`
- [ ] Error: inline Deep Rose (#E11D48) underline + clear error text
...
- [ ] Dynamic per-delegate blocks with `fadeUp` entrance and `AnimatePresence` exit
- [ ] Applicant type toggle with `layoutId` sliding indicator

### 3.3 Moot Cup Registration Form — PRD §17.2

Team-based (2-4 members): Team Name, Institution, per-member blocks (Name/Email/Phone), Problem Category Preference, Moot Experience, Dietary Notes, Referral Source
...
- [ ] **Error shake animation** — 3 oscillations over 400ms using Framer Motion spring (per animation-vocabulary "shake/wiggle")

**Non-Payment Statement (PRD §6.2):**
- [ ] Prominently displayed above submit button
- [ ] Appears in confirmation email

**Submission Flow:**
- [ ] Success: styled confirmation panel replaces form with `scaleIn` animation
- [ ] Confirmation email to applicant + notification to OC
- [ ] Failure: user-friendly error, form data preserved

**Closed-Registration State:**
- [ ] Form replaced with clear closed message when `registrationOpen` is false in `site.json`

**Bot Protection:**
- [ ] Honeypot field + optional CAPTCHA + rate limiting

### 3.5 Form Backend

- [ ] Next.js API Route or managed form backend (Formspree/Getform)
- [ ] Google Sheets integration + email notifications

### 3.6 Contact Form (`/contact`) — PRD §19.3

- [ ] Fields: Name, Email, Query Type (GIMUN/Moot Cup/Sponsorship/Media/Other), Message
- [ ] Contact details alongside form, social media links with Lucide icons

### 3.7 Clarification Submission Form

- [ ] Team Name, Email, Question — routes to OC email

### 3.8 Phase 3 Acceptance Criteria

- [ ] No payment fields anywhere
- [ ] Delegation head can submit entire delegation in one form
...
- [ ] **stitch-design-taste form styling applied**: label above, accent focus ring, contextual errors
